...
## 데일리 시드 + 리플레이 (공정성·검증)
> **구현 현황**: **리플레이 기록·재생은 구현됨**(`src/replay/replay.ts`) — 단 설계서 원안(시드+플레이어입력만)과 달리, **모든 tick의 `(dt, intents)` 스트림을 통째로 기록**하는 방식을 택했다. AI 타이밍이 벽시계 기반이라 "플레이어 입력만 + AI 재생성"은 타이밍 재현이 어렵지만, 전체 입력열을 잡으면 AI 수까지 인텐트로 포함돼 **tick 순수성만으로 bit-exact 재현**이 보장된다(테스트 `replay.test.ts`). 저장은 localStorage(최근 5) + JSON 다운로드.
> **아직 후순위**: `dailySeed`(날짜 시드)·서버 `verify` 재시뮬 검증·리더보드·하루 3판 — 게임이 더 여문 뒤.
>
> 아래는 원래 설계(서버 검증까지)의 참고 스케치다.
